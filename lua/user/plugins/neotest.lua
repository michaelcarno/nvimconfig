...
return {
  ...
  },
  {
    ...
    keys = {
      ...
      {
        prefix .. "f",
        function() require("neotest").run.run(vim.fn.expand(vim.fn.expand "%")) end,
        desc = "Run file",
      },
      { prefix .. "d", function() require("neotest").run.run { strategy = "dap" } end, desc = "Debug test file" },
      {
        prefix .. "w",
        function() require("neotest").watch.toggle(vim.fn.expand(vim.fn.expand "%")) end,
